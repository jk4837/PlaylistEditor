...
if ($self -eq $true) {
    $command += " | Select-String -pattern `"(QQQQQ|playlisteditor|AndroidRuntime|CRASH)`""
    # $command += " | Select-String -pattern `"(playlisteditor|SongLoader|PlayListEditor|AndroidRuntime|CRASH)`""
} elseif ($custom -ne "") {
